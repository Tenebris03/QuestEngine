...
const PreferenceForm: React.FC<PreferenceFormProps> = ({ onSave, onClose, initialValues }) => {
  const { t } = useTranslation('preferenceForm');
  ...
  return (
    <div className="preference-form-overlay" role="dialog" aria-modal="true" aria-labelledby="pref-title">
      <div className="preference-form">
        <h2 id="pref-title" className="form-title">{t('title')}</h2>
        <p className="form-subtitle">{t('subtitle')}</p>

        {/* Fitness-Ziel */}
        <div className="form-group">
          <label htmlFor="fitness-goal" className="form-label">{t('fitnessGoal.label')}</label>
          <select id="fitness-goal" className="form-select" value={goal} onChange={(e) => setGoal(e.target.value as UserPreferences['fitnessGoal'])}>
            <option value="strength">{t('fitnessGoal.options.strength')}</option>
            <option value="endurance">{t('fitnessGoal.options.endurance')}</option>
            <option value="weightloss">{t('fitnessGoal.options.weightloss')}</option>
            <option value="muscle">{t('fitnessGoal.options.muscle')}</option>
          </select>
        </div>

        {/* Verfügbare Zeit */}
        <div className="form-group">
          <label htmlFor="available-time" className="form-label">{t('availableTime.label')}</label>
          <select id="available-time" className="form-select" value={time} onChange={(e) => setTime(Number(e.target.value) as UserPreferences['availableTime'])}>
            <option value={30}>{t('availableTime.options.30')}</option>
            <option value={45}>{t('availableTime.options.45')}</option>
            <option value={60}>{t('availableTime.options.60')}</option>
            <option value={90}>{t('availableTime.options.90')}</option>
            <option value={120}>{t('availableTime.options.120')}</option>
          </select>
        </div>

        {/* Erfahrungslevel */}
        <div className="form-group">
          <label htmlFor="experience-level" className="form-label">{t('experienceLevel.label')}</label>
          <select id="experience-level" className="form-select" value={level} onChange={(e) => setLevel(e.target.value as UserPreferences['experienceLevel'])}>
            <option value="beginner">{t('experienceLevel.options.beginner')}</option>
            <option value="intermediate">{t('experienceLevel.options.intermediate')}</option>
            <option value="advanced">{t('experienceLevel.options.advanced')}</option>
          </select>
        </div>
        ...
        <div className="form-group">
          <label htmlFor="intensity" className="form-label">
            {t('intensity.label')}: <span className="intensity-value">{t('intensity.value', { value: intensity })}</span>
          </label>
          <input
            id="intensity"
            type="range"
          ...
          <div className="slider-labels">
            <span>{t('intensity.min')}</span>
            <span>{t('intensity.max')}</span>
          </div>
        </div>

        {/* Equipment */}
        <fieldset className="form-group equipment-group">
          <legend className="form-label">{t('equipment.label')}</legend>
          <div className="equipment-options">
            {EQUIPMENT_OPTIONS.map((item) => (
              <label key={item.value} className="equipment-checkbox">
                ...
                />
                <span>{t(`equipment.options.${item.key}`)}</span>
              </label>
            ))}
          </div>
        </fieldset>
